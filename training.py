...
def config():
    ...
    comments = 'allns_36_embproj_nogradreset_dropout:.3_timerepeat:2_findLSC_radius_test_onlypretrain_pretrained_lsclr:0.0001_nbs:16_tsteps:10_shuffwsometimes'
...
def main(epochs, steps_per_epoch, batch_size, GPU, task, comments,
    ...
    for _ in range(relsc):
        if 'findLSC' in comments or 'onlyloadpretrained' in comments:
            ...
            # lsclr = 1e-2
            # lsclr = 3.14e-2
            lsclr = 3.14e-3 # 3.14e-3 # 7.2e-4 # 7.2e-3 #
            # if 'LSNN' in net_name:
            #     lsclr = 3.14e-4
