...
class non_spiking_LIF(tf.keras.layers.Layer):
    ...
    def __init__(self, num_neurons=None, initializer='orthogonal',
                 config='', v_eq=0,
                 **kwargs):
        super().__init__(**kwargs)

        self.init_args = dict(num_neurons=num_neurons, initializer=initializer, )
    ...
    def build(self, input_shape):
        ...
        self.internal_current = self.add_weight(shape=(n_rec,), initializer='zeros', name='bias')

        self.built = True

    def currents_composition(self, inputs, old_spike):
        i_in = inputs @ self.input_weights \
               + old_spike @ self.recurrent_weights \
    ...
    def call(self, inputs, states, training=None):
        ...
        v = self.currents_composition(inputs, old_v)
...
class aLSNN(baseLSNN):
    ...
    def build(self, input_shape):
        ...
        trainable = str2val(self.config, 'trainablehypers', bool, default=False)
        ...
        for k, p in parameter2trainable.items():
            ...
            p = self.add_weight(shape=(self.num_neurons,), initializer=initializer, name=k, trainable=trainable)
        ...
        if 'LSC1' in self.config:
            ...
            alpha_a = .99  # 1/3 .86
            tau_adaptation = 700  # -1 / tf.math.log(alpha_a)
            self.tau_adaptation = self.add_weight(shape=(self.num_neurons,),
                                                  ...
                                                  name='tau_adaptation', trainable=trainable)
        ...
        elif 'veryrandom' in self.config:
            self.tau = self.add_weight(shape=(self.num_neurons,),
                                       initializer=
                                       tf.keras.initializers.RandomUniform(minval=-1.2, maxval=1.2),
                                       name='tau', trainable=trainable)
            self.tau_adaptation = self.add_weight(shape=(self.num_neurons,),
                                                  initializer=
                                                  tf.keras.initializers.RandomUniform(minval=-1.2, maxval=1.2),
                                                  name='tau_adaptation', trainable=trainable)
            self.dampening = tf.random.uniform((self.num_neurons,), minval=0.2, maxval=1.2)
            self.beta = self.add_weight(shape=(self.num_neurons,),
        ...
        # print(            f'ta: {self.tau_adaptation[0].numpy}, t: {self.tau[0].numpy}, '
        #                   f'beta: {self.beta[0].numpy}, thr: {self.thr[0].numpy}')
